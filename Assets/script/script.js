...
const smallCanvas = document.createElement('canvas'); // Smaller canvas for color extraction
const smallCtx = smallCanvas.getContext('2d');
const colorThief = new ColorThief();
let currentBackgroundColor = [0, 0, 0]; // Initialize background color
const colorHistory = [];
...
videoPlayer.addEventListener('loadedmetadata', () => {
  ...
  smallCanvas.width = videoPlayer.videoWidth / 4; // Smaller canvas
  smallCanvas.height = videoPlayer.videoHeight / 4;
});
let GradientState = null; // Initialize to null

videoPlayer.addEventListener('timeupdate', () => {
    if (TheaterModeFlag) {
        if (GradientState === null) { // Check if an interval is already running
            GradientState = setInterval(function () {
                smallCtx.drawImage(videoPlayer, 0, 0, smallCanvas.width, smallCanvas.height);
                const dominantColor = colorThief.getColor(smallCanvas);

                colorHistory.push(dominantColor);
                if (colorHistory.length > historySize) {
                    colorHistory.shift();
                }

                const averageColor = averageColors(colorHistory);

                const lerpFactor = 0.6;
                currentBackgroundColor[0] = lerp(currentBackgroundColor[0], averageColor[0], lerpFactor);
                currentBackgroundColor[1] = lerp(currentBackgroundColor[1], averageColor[1], lerpFactor);
                currentBackgroundColor[2] = lerp(currentBackgroundColor[2], averageColor[2], lerpFactor);

                const smoothedColorHex = rgbToHex(
                    Math.round(currentBackgroundColor[0]),
                    Math.round(currentBackgroundColor[1]),
                    Math.round(currentBackgroundColor[2])
                );
                ...
                gradient.addColorStop(0, smoothedColorHex);
        ...
        } // End of if (GradientState === null)
    } else {
        if (GradientState !== null) { // Check if an interval is running before clearing
            clearInterval(GradientState);
            GradientState = null; // Reset GradientState
            body.style.backgroundImage = 'url(Other_Files/bg-tv.png)';
        }
    }
});